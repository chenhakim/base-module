...
class SdkPayment
{
    ...
    )
    {
        $parameter = array(
            ...
            'batch_num' => 1,
            'detail_data' => $transaction_id . '^' . $refund_amount . '^' . $refund_reason,
            'return_type' => 'xml',
        );
        $para = $this->buildRequestPara($parameter);

        $refund_url = $this->__gateway_new . $this->createLinkstringUrlencode($para);

        $response_txt = $this->getHttpResponseGET($refund_url, $this->cacert);

        if (strpos(strtolower($response_txt), '<is_success>T</is_success>') != -1) {
            return true;
        } else {
            return false;
        }
